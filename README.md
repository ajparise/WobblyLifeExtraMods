...
An extension plugin for the installed **lstwoMODS Wobbly Life** mod menu. Version 1.5.1 adds these panels to the existing **Extra Mods** page:
...
For realistic crashes, open **Extra Mods → Realistic Car Crashes** and choose **Enable realistic crashes**. Road vehicle impacts below the minimum threshold retain the game's normal behavior. Faster direct impacts apply capped native vehicle damage and remove forward momentum, while off-center contact adds rotation based on the collision point. Light posts, traffic lights, signs, bollards, and other low-mass street props are treated as minor obstacles and do not add crash damage. At the severe-crash threshold, occupants ragdoll and receive impact velocity. Minimum and severe speeds, damage, damage cap, momentum loss, spin, minor-prop filtering, and occupant ragdolls are adjustable. Synchronized vehicle damage requires an offline game or the lobby host.
...
Check `<Wobbly Life>\BepInEx\LogOutput.log` for `Wobbly Life Extra Mods 1.5.1 loaded`. If a searchable panel is empty, wait for the lstwoMODS asset scan to finish and click its refresh button.
